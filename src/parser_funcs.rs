...
pub fn parse_ini_to_avd(path: String) -> AvdItem {
    ...
    for line in contents.lines() {
        ...
        match pair[0].trim() {
            ...
            _FASTBOOT_CHOSEN_SNAPSHOT_FILE_KEY => {
                fastboot_chosen_snapshot_file = pair[1].trim().to_string()
            }
            _FASTBOOT_FORCE_CHOSEN_SNAPSHOT_BOOT_KEY => {
                fastboot_force_chosen_snapshot_boot = pair[1].trim() == "yes"
            }
...
pub fn parse_avd_to_ini(item: AvdItem) -> String {
    ...
    out += &get_parsed_value(
        _FASTBOOT_CHOSEN_SNAPSHOT_FILE_KEY,
        &item.fastboot_chosen_snapshot_file,
    );
    out += &parse_bool_to_ini(
        _FASTBOOT_FORCE_CHOSEN_SNAPSHOT_BOOT_KEY,
        item.fastboot_force_chosen_snapshot_boot,
        true,
    );
    out += &parse_bool_to_ini(
        _FASTBOOT_FORCE_COLD_BOOT_KEY,
        item.fastboot_force_cold_boot,
        true,
    );
    out += &parse_bool_to_ini(
        _FASTBOOT_FORCE_FAST_BOOT_KEY,
        item.fastboot_force_fast_boot,
        true,
    );
    ...
    out += &parse_bool_to_ini(
        _HW_SENSORS_ORIENTATION_KEY,
        item.hw_sensors_orientation,
        true,
    );
...
fn parse_bool_to_ini(key: &str, value: bool, yes_no: bool) -> String {
    let parsed_bool: String = match yes_no {
        true => String::from(match value {
            true => "yes",
            _ => "no",
        }),
        _ => String::from(match value {
            true => "true",
            _ => "false",
        }),
    };
